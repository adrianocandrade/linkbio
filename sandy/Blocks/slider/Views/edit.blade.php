...
<form method="post" action="{{ route('user-mix-block-element-edit', $block->id) }}" class="mix-padding-10 w-full" enctype="multipart/form-data">
  ...
  <div class="grid grid-cols-2 gap-4 mb-5">
    ...
    <div class="form-input">
      ...
      <input type="text" name="content[caption]" value="{{ ao($block->content, 'caption') }}">
    </div>
    <div class="form-input">
      <label>{{ __('Describe the image') }}</label>
      <input type="text" name="content[alt]" value="{{ ao($block->content, 'alt') }}">
    </div>
  </div>

    <div class="h-avatar h-32 w-32 is-upload is-outline-dark text-2xl mb-5 sandy-upload-modal-open">
      <?= media_or_url($block->thumbnail, 'media/blocks', true) ?>

      <div class="sandy-file-name"></div>
    </div>
  {!! Blocks::LinkOrElementHtml($user->id, ['is_element' => (int) $block->is_element, 'element_id' => $block->element, 'link' => $block->link]) !!}
