...
<div class="context bio a-left">
    ...
    <div class="context-head px-5 md:px-10 pt-10 pb-0 mt-0">
        ...
        <div class="shop-name-text text-2xl theme-text-color flex">{{ user('settings.course.name', $bio->id) }}</div>
        <div class="bio-des mt-5 text-sm mb-7 theme-text-color">{{ user('settings.course.description', $bio->id) }}</div>
    </div>
    <div class="mix-padding-10">
        @if (!$courses->isEmpty())
        <h1 class="my-7 font-bold">{{ __('Courses') }}</h1>
