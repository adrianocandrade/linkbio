<div class="element-video">
    ...
    <div class="relative flex flex-col gap-4 {{ empty($this->blocks) ? 'hidden' : '' }}" wire:sortable="sort">
        ...
        <div class="yetti-popup-wrapper"  x-data="mixing" wire:sortable.item="{{ ao($item, 'id') }}" wire:key="embed-blocks-{{ ao($item, 'id') }}">
      
            <div class="flex">
                    
                <div class="w-auto flex p-2 rounded-full mb-2 bg-gray-100 ml-auto">
                    
                    <p class="drag mr-0 px-5 handle cursor-pointer" wire:sortable.handle>
                        <i class="sni sni-move text-black"></i>
                    </p>
                    <a class="ml-auto px-5 pointer-events-auto cursor-pointer" x-on:click="open">
                        <i class="sni sni-pen text-black"></i>
                    </a>
                    <button wire:click="delete_block({{ ao($item, 'id') }})" class="px-5">
                    ...
                    <form class="m-0" wire:submit.prevent="edit({{ ao($item, 'id') }}, {{ $index }})" enctype="multipart/form-data">
                        ...
                        <div class="wj-image-selector-w is-avatar relative active">
                            <a class="wj-image-selector-trigger rounded-xl flex items-center relative">
                                ...
                                <div wire:loading wire:target="thumbnail" class="text-xs font-bold m-0 absolute -bottom-6 left-0">{{ __('Processing image...') }}</div>
                            </a>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            ...
                            <div class="form-input">
                              ...
                              <input type="text" wire:model.defer="blocks.{{ $index }}.content.subheading">
                            </div>
                        </div>
