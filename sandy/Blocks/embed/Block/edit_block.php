<?php
    $fetch = ao($block->blocks, 'fetch');


    $js_function = "sandy_embed_id_$block->id";
...
<div class="relative z-10">
    ...
    <?php if (\Auth::check() && \Auth::user()->id == $block->user): ?>
    <form action="<?= route('sandy-blocks-embed-retrieve') ?>" method="post" class="mt-3">
        <input type="hidden" name="_token" value="<?= csrf_token() ?>">
        <input type="hidden" name="block" value="<?= $block->id ?>">
        
        <button class="text-sticker secondary-box rounded-2xl"><?= __('Fetch new info') ?></button>
    </form>
    <?php endif ?>
</div>
