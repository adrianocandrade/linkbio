...
@section('content')
<form action="{{ route("sandy-app-embed_links-edit", $element->slug) }}" class="mix-padding-10" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mt-5 mort-main-bg rounded-2xl p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
            ...
            <label class="sandy-big-checkbox">
                <input type="radio" name="content[type]" class="sandy-input-inner" data-placeholder-input="#video-link" data-placeholder="{{ ao($value, 'placeholder') }}" {{ ao($element->content, 'type') == $key ? 'checked' : '' }} value="{{ $key }}">
        ...
        <div class="form-input mb-5">
            ...
            <input type="text" placeholder="{{ __('Label this element') }}" class="bg-w" value="{{ $element->name }}" name="label">
        </div>
        <div class="form-input mb-2">
            <label class="initial">{{ __('URL Link') }}</label>
            <input type="text" name="content[link]" value="{{ ao($element->content, 'link') }}" class="bg-w" id="video-link">
        </div>
