...
@section('content')
<form action="{{ route('sandy-app-guestbook-edit', $element->slug) }}" method="post">
    @csrf
            ...
            <div class="card customize m-0 p-0">
                <div class="card-header">
                    <div class="form-input mb-7 text-count-limit" data-limit="60">
                        ...
                        <input type="text" name="name" class="bg-w" value="{{ $element->name }}">
                    </div>
                    <div class="form-input text-count-limit" data-limit="200">
                        <label>{{ __('Description') }}</label>
                        <span class="text-count-field"></span>
                        <textarea rows="4" name="description" class="bg-w">{{ ao($element->content, 'description') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
