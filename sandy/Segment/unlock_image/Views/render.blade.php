...
<div class="context bio h-screen p-0 {!! radius_and_align_class($bio->id, 'align') !!} is-element flex flex-col">
    ...
    <div class="element-context-body remove-before mt-10 pb-0 relative">
        <div class="bio-background">
            <img data-src="{{ route('sandy-app-unlock_image-render-image-blured', $element->slug) }}" class="bio-background-inner lozad image" alt=" ">
        </div>
        <div class="message">
            ...
            <button class="button sandy-quality-button mt-0 is-loader-submit loader-white relative unlock-open">{!! __('Unlock for :price', ['price' => Currency::symbol(ao($bio->payments, 'currency')) . ao($element->content, 'price')]) !!}</button>
        </div>
    </div>
</div>
<div data-popup=".unlock">
    <form method="post" action="{{ route('sandy-app-unlock_image-unlock', $element->slug) }}">
        @csrf
