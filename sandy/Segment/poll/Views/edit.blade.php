...
@section('content')
<form action="{{ route("sandy-app-poll-edit", $element->slug) }}" class="mix-padding-10" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mt-5 mort-main-bg rounded-2xl p-5">
        ...
        <div class="form-input mb-7">
            ...
            <textarea name="content[question]" class="bg-w">{{ ao($element->content, 'question') }}</textarea>
        </div>
        <div class="bg-white p-5 rounded-2xl relative">
            <div class="not-plan">
                <div class="content">
                    <div class="p-10 flex justify-center">
                        {{ __('Poll’s cannot be edited') }}
                    </div>
                </div>
                <div class="opacity-reduce rounded-2xl"></div>
            </div>
                ...
                @foreach ($choices as $key => $value)
                    <div class="flex mb-5">
                        ...
                        <div class="form-input w-full mr-4">
                            ...
                            <input type="text" data-item-name="choice" value="{{ ao($value, 'name') }}" name="choices[{{$key}}][choice]">
                        </div>
