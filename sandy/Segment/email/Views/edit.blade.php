...
@section('content')
@section('title', __('Email Collection'))
<form action="{{ route('sandy-app-email-edit', $element->slug) }}" method="post">
    @csrf
        ...
        <div class="card customize bg-white mb-10">
            <div class="card-header">
                <div class="form-input mb-7 text-count-limit" data-limit="60">
                    ...
                    <input type="text" name="title" value="{{ ao($element->content, 'title') }}">
                </div>
                <div class="form-input text-count-limit" data-limit="200">
                    <label class="initial block mb-3">{{ __('Description') }}</label>
                    <span class="text-count-field"></span>
                    <textarea rows="4" name="description" class="text-xs" placeholder="{{ __('Ex: Sign up with your email address to receive news and updates.') }}">{{ ao($element->content, 'description') }}</textarea>
                </div>
                <div class="card customize mt-5">
                  <div class="card-header">
                    ...
                    <div class="custom-switch mt-3">
                      ...
                      <input type="checkbox" class="custom-control-input" name="require_name" id="require-name" value="1" {{ ao($element->content, 'require_name') ? 'checked' : '' }}>
                ...
                <div class="form-input text-count-limit mt-5" data-limit="200">
                    ...
                    <span class="text-count-field"></span>
                    <textarea rows="2" class="text-xs" placeholder="{{ __('Ex: Your personal information will not be sold to advertisers or other Third Parties.') }}" name="disclaimer">{{ ao($element->content, 'disclaimer') }}</textarea>
                </div>
            </div>
        </div>
    </div>
