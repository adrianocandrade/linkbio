...
<div class="context bio h-screen p-0 {!! radius_and_align_class($bio->id, 'align') !!} is-element flex flex-col">
    ...
    <div class="element-context-body mt-10 pb-0">
        ...
        <div class="message">
            ...
            @if (ao($element->content, 'price.type') == 'fixed')
            <button class="button sandy-quality-button mt-0 is-loader-submit loader-white relative purchase-open">{!! __('Unlock for :price', ['price' => Currency::symbol(ao($bio->payments, 'currency')) . ao($element->content, 'price.price')]) !!}</button>
            @else
            <form method="post" class="w-full" action="{{ route('sandy-app-unlock_video-purchase', $element->slug) }}">
                @csrf
                ...
                <button class="button sandy-quality-button mt-0 is-loader-submit loader-white relative">{!! __('Unlock') !!}</button>
            </form>
            @endif
        </div>
    </div>
...
<div data-popup=".purchase">
    <form method="post" action="{{ route('sandy-app-unlock_video-purchase', $element->slug) }}">
        @csrf
