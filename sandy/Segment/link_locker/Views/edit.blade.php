...
@section('content')
<form action="{{ route("sandy-app-link_locker-edit", $element->slug) }}" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="p-10">
        ...
        <div class="form-input mb-7">
            ...
            <textarea name="content[description]">{{ ao($element->content, 'description') }}</textarea>
        </div>
        <div class="form-input mb-2">
            <label>{{ __('URL Link') }}</label>
            <input type="text" name="content[link]" value="{{ ao($element->content, 'link') }}">
        </div>
        ...
        <div class="form-input is-link mb-7 always-active active">
            ...
            <div class="is-link-inner">
                ...
                <input type="text" name="content[price]" value="{{ ao($element->content, 'price') }}">
            </div>
        </div>
