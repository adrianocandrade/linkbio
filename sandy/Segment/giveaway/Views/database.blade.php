...
<div>
    ...
    <div class="inner-page-banner">
        ...
        <div class="mt-5">{!! Elements::icon('giveaway') !!}</div>
        <h1 class="mt-5 text-base">{{ Elements::config('giveaway', 'name') }}</h1>

        <div class="flex">
            <a href="{{ route('sandy-app-giveaway-export', $element->slug) }}" app-sandy-prevent="" class="text-sticker mt-5">{{ __('Export To CSV') }}</a>
        </div>
    </div>
    <div class="p-10">
        ...
        <div class="flex-table mt-4">
            ...
            <div class="flex-table-item rounded-2xl">
                ...
                <div class="flex-table-cell" data-th="{{ __('Date Of Birth') }}">
                    <span>{{ ao($item->database, 'dob') }}</span>
                </div>
                <div class="flex-table-cell" data-th="{{ __('Phone') }}">
                    <span>{{ ao($item->database, 'phone') }}</span>
                </div>
            </div>
            @endforeach
        </div>
