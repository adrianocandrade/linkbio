@if (!$cart->isEmpty())
    ...
    <div class="product-cart">
        ...
        <div class="content">
            <a class="text-base" href="{{ \Bio::route($bio->id, 'user-bio-shop-single-product', ['id' => $item->associatedModel->id]) }}">{{ $item->name }}</a>
        ...
        <form action="{{ \Bio::route($bio->id, 'user-bio-shop-cart-remove-item') }}" method="post">
            @csrf
...
<div class="no-record">
    <div class="rounded-3xl block">
        <div class="text-center flex justify-center flex-col items-center">
            ...
            <a href="{{ \Bio::route($bio->id, 'user-bio-shop-home') }}" class="mt-5 sandy-expandable-btn rounded-lg sandy-loader-flower"><span>{{ __('Start Shopping') }}</span></a>
        </div>
    </div>
</div>
@endif