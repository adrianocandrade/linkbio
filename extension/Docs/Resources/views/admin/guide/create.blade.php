    ...
    <div class="page__head">
      ...
      <div class="section-header mb-16">
        <div class="section-header-info">
          <p class="section-pretitle">{{ __('New Guide') }}</p>
          <h2 class="section-title">{{ __('Guide') }}</h2>
        </div>
        <div class="section-header-actions">
          <a href="{{ route('admin-blogs') }}" class="section-header-action">{{ __('All Guides') }}</a>
        </div>
      </div>
    </div>
    <form method="post" action="{{ route('admin-docs-guide-create-post', $docs->id) }}" enctype="multipart/form-data">
      @csrf
          ...
          <div class="card cuztomize mb-5 p-5 mort-main-bg rounded-2xl">
            ...
            <div class="grid md:grid-cols-2 gap-4 mt-5">
              <div class="form-input mt-5">
                ...
                <input type="text" class="bg-w" value="{{ old('short_des') }}" name="short_des">
              </div>
              <div class="form-input mt-5">
                <label class="initial">{{ __('Docs Category') }}</label>
                <select name="docs" id="" class="bg-w">
                  @foreach ($alldocs as $item)
                  <option value="{{ $item->id }}" {{ $docs->id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-input mt-5">
