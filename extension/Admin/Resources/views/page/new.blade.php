      ...
      var objects = {
          changeInput: function ($this) {
            switch($this.find(':selected').attr("value")) {
              case "internal":
               jQuery('#page-slug').find('label').html("{{ __('Slug') }}");
               jQuery('.input-side-pre').show();
               jQuery('#page-slug').addClass('pre');
              break;
              case "external":
               jQuery('#page-slug').find('label').html("{{ __('External Url') }}");
               jQuery('.input-side-pre').hide();
               jQuery('#page-slug').removeClass('pre');
              break;
            }
          }
      }
      ...
      <div class="step-banner remove-shadow">
        <div class="section-header">
          <div class="section-header-info">
            ...
            <h2 class="section-title">{{ __('New Page') }}</h2>
          </div>
          <div class="section-header-actions">
            <a href="{{ route('admin-pages') }}" class="section-header-action">{{ __('All Pages') }}</a>
          </div>
        </div>
      </div>
    </div>
    <form method="post" action="{{ route('admin-new-page-post') }}" class="rounded-3xl card-shadow p-5" enctype="multipart/form-data">
      @csrf
      <div class="card cuztomize mb-5 p-5 mort-main-bg rounded-2xl">
        <div class="form-input">
          <label>{{ __('Name') }}</label>
          <input type="text" class="bg-w" value="{{ old('name') }}" name="name">
        </div>
        <div class="grid grid-cols-2 gap-4 mt-5">
          <div class="form-input mt-5">
            <label class="initial">{{ __('Page type') }}</label>
            <select name="type" id="" class="bg-w">
      ...
      <div class="mb-5 p-5 mort-main-bg rounded-2xl">
        <div class="form-input pre" id="page-slug">
          ...
          <div class="flex">
            ...
            <input type="text" name="location" class="bg-w">
          </div>
        </div>
      </div>
      <div class="mb-5 p-5 mort-main-bg rounded-2xl">
        <div class="flex items-center flex-col">
          ...
          <div class="h-avatar h-52 w-full is-upload is-outline-dark text-2xl" data-generic-preview=".h-avatar">
            ...
            <div class="image lozad" data-loaded="true"></div>
          </div>
        </div>
      </div>
      <div class="my-5 p-5 card mort-main-bg rounded-2xl">
        <div class="form-input">
          ...
          <textarea name="description" class="editor" id="" cols="30" rows="10"></textarea>
        </div>
      </div>
      <div class="p-5 mort-main-bg mb-5 rounded-2xl">
        <div class="form-input">
          <label>{{ __('Author') }}</label>
          <input type="text" name="author" value="{{ user('name') }}" class="bg-w">
        </div>
      </div>
      <div class="p-5 mort-main-bg rounded-2xl">
        <div class="form-input">
          <label>{{ __('Time to read. Ex: 5 min') }}</label>
          <input type="text" name="ttr" class="bg-w">
        </div>
      </div>
  ...
  <div class="sandy-page-col sandy-page-col_pt100 md:px-10">
    <div class="card card_widget">
      ...
      <div class="sandy-quality">
        ...
        @endforelse
        <a class="button sandy-quality-button bg-black" href="{{ route('admin-pages') }}">{{ __('All Pages') }}</a>
      </div>
    </div>
  </div>
</div>
