...
<div class="sandy-page-row">
  <div class="sandy-page-col pl-0">
      ...
      <div class="step-banner remove-shadow">
        <div class="section-header">
          <div class="section-header-info">
            ...
            <h2 class="section-title">{{ __('Add Plan') }}</h2>
          </div>
    ...
    <form method="post" action="{{ route('admin-new-plan-post') }}" class="rounded-3xl card-shadow p-8">
      @csrf
      
      <div class="card cuztomize mb-5 p-5 mort-main-bg rounded-2xl">
        <div class="form-input">
          <label for="">{{ __('Name') }}</label>
          <input type="text" name="name" value="{{ old('name') }}" class="bg-w">
        </div>
        <div class="form-input mt-5">
          <label class="initial">{{ __('Plan type') }}</label>
          <select name="type" id="" class="bg-w" data-sandy-select=".select-shift">
            <option value="paid">{{ __('Paid') }}</option>
            <option value="free">{{ __('Free forever') }}</option>
        ...
        <div class="select-shift mt-5">
          <div data-sandy-open="paid">
            <div class="grid grid-cols-2 gap-4">
              <div class="form-input">
                <label for="">{{ __('Monthly Price') }}</label>
                <input type="text" name="price[monthly]" class="bg-w">
              </div>
              <div class="form-input">
                <label for="">{{ __('Annual Price') }}</label>
                <input type="text" name="price[annually]" class="bg-w">
              </div>
            </div>
          </div>
          ...
          <div data-sandy-open="trial">
            <div class="form-input">
              <label for="">{{ __('Trial duration') }}</label>
              <input type="text" name="price[trial_duration]" class="bg-w">
            </div>
          </div>
        </div>
        <div class="form-input mt-5">
          <label class="initial">{{ __('Status') }}</label>
          <select name="status" id="" class="bg-w">
        ...
        <div class="form-input mt-5 text-count-limit" data-limit="250">
          ...
          <textarea name="extra[description]" class="bg-w" cols="30" rows="10"></textarea>
        </div>
          ...
          <div class="card customize mt-5 mb-0">
            <div class="card-header">
              ...
              <div class="custom-switch mt-3">
                ...
                <input type="checkbox" class="custom-control-input" name="extra[featured]" id="plan-featured" value="1" >
          ...
          <div class="card customize mt-5 mb-0">
            <div class="card-header">
              ...
              <div class="custom-switch mt-3">
                ...
                <input type="checkbox" class="custom-control-input" name="defaults" id="plan-default" value="1">
      ...
      <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <label class="sandy-big-checkbox">
          ...
          <input type="checkbox" name="settings[{{$key}}]" class="sandy-input-inner" value="1">
      ...
      <div class="my-5 p-5 card mort-main-bg rounded-2xl">
        <div class="grid grid-cols-2 gap-4">
          <div class="form-input">
            <label for="">{{ __('Blocks Limit') }}</label>
            <input type="text" name="settings[blocks_limit]" class="bg-w">
          </div>
          <div class="form-input">
            <label for="">{{ __('Workspaces Limit') }}</label>
            <input type="text" name="settings[workspaces_limit]" class="bg-w">
            <p class="italic mt-4 text-xs">{{ __('(Note: Min is 1)') }}</p>
          </div>
          <div class="form-input">
            <label for="">{{ __('Pixel Limit') }}</label>
            <input type="text" name="settings[pixel_limit]" class="bg-w">
          </div>
        </div>
      </div>
