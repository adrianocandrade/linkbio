...
<form class="mix-padding-10 profile-form" method="post" action="{{ route('user-mix-settings-post', 'profile') }}" enctype="multipart/form-data">
	...
	<div class="mb-10">
		<div class="form-input is-link always-active active mb-7">
			...
			<input type="text" name="username" placeholder="{{ __('Input a username') }}" class="is-alt-input bg-white" value="{{ $account->username }}">
			</div>
		</div>
		<div class="form-input mb-7">
			<label>{{ __('Name') }}</label>
			<input type="text" name="name" class="bg-w-" value="{{ $account->name }}">
		</div>
		<div class="form-input mb-7">
			<label>{{ __('Email') }}</label>
			<input type="email" name="email" class="bg-w-" value="{{ $account->email }}">
		</div>
		<div class="form-input">
			<label>{{ __('Bio') }}</label>
			<textarea rows="4" name="bio" class="bg-w-">{{ $account->bio }}</textarea>
		</div>
